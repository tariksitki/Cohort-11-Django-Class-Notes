...
def home(request):
    ...
    ## COOKIES: HTTP-Cookies sind kleine Datenblöcke, die von einem Webserver erstellt werden, während ein Benutzer auf einer Website surft, und vom Webbrowser des Benutzers auf dem Computer oder einem anderen Gerät des Benutzers abgelegt werden.
    #
    # Wenn Sie eine Website besuchen, die Cookies verwendet, wird eine Cookie-Datei auf Ihrem PC, Mac, Telefon oder Tablet gespeichert. Es speichert den Namen der Website sowie eine eindeutige ID, die Sie als Benutzer repräsentiert. Auf diese Weise weiß die Website, wenn Sie diese Website erneut besuchen, dass Sie bereits zuvor dort waren.
    #
    ...
    ## Ein CSRF-Token ist ein eindeutiger, geheimer, unvorhersehbarer Wert, der von der serverseitigen Anwendung generiert und so an den Client übertragen wird, dass er in einer nachfolgenden HTTP-Anfrage des Clients enthalten ist.
    ...
    # print(request.path)
    # /
    ## Bu kod bize;  base url den sonra gelen endpoint i verir
...
Django verwendet Request- und Response-Objekte, um den Zustand durch das System zu leiten.

Wenn eine Seite angefordert wird, erstellt Django ein HttpRequest-Objekt, das Metadaten über die Anforderung enthält. Dann lädt Django die entsprechende Ansicht und übergibt HttpRequest als erstes Argument an die Ansichtsfunktion. Jede Ansicht ist für die Rückgabe eines HttpResponse-Objekts verantwortlich.
...
## NGINX:
Es ist der schnellste Webserver der Welt. Es wurde von einem russischen Ingenieur für mail.ru entwickelt, wird aber aufgrund seiner skalierbaren Struktur in vielen Bereichen eingesetzt. Es bietet 400 % mehr Leistung als seine Alternativen. Es wird insbesondere auf Websites verwendet, die eine hohe Leistung erfordern, wie z. B. Google Netflix. Es basiert auf der Bereitstellung hoher Leistung mit weniger CPU. Die Verwendungszwecke sind:
1: Lastausgleich
2: Reverse-Proxy
3: virtueller Server
4: Präsentation von statischen und Indexdateien, automatische Indizierung.
